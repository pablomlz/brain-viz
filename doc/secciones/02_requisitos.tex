...
de roles. Para abreviar se emplean las siglas \textbf{[V]} (visitante),
...
\noindent\textbf{[RNF 7]} La interfaz deberá adaptarse al tamaño de la pantalla del dispositivo, de
manera que la aplicación resulte utilizable tanto en un ordenador como en un teléfono móvil, en
cualquiera de sus dos orientaciones, sin que los paneles de información lleguen a ocultar la
visualización ni queden fuera de la parte aprovechable de la pantalla.
...
que interactúan con ella directamente. En una primera fase se presentan los diagramas de casos
...
requisitos. Además, al finalizar cada sprint se realizará una validación por parte del cliente, papel
que en este proyecto asume el director, para lo cual la aplicación se desplegará en un servidor
accesible.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
		...
		4 & RF 11 -- RF 18 &
			...
			administrador; exportación; la interfaz se adapta a la pantalla de un móvil sin que los
			paneles oculten la escena [RNF 7]. \\
		\bottomrule
	\end{tabularx}
\end{table}
